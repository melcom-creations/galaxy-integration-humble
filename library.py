...
class LibraryResolver:
    ...
    async def __call__(self, only_cache: bool = False) -> Dict[str, HumbleGame]:
        ...
        # Load games in the order stored in the cache.
        ...
        # Merge games that share the same title.
    ...
    def _get_subproducts(orders: list) -> List[Subproduct]:
        ...
        for details in orders:
            for sub_data in details['subproducts']:
                ...
                try:
                    sub.in_galaxy_format()  # Validate the Galaxy-specific format before using the subscription.
                ...
                if not set(sub.downloads).isdisjoint(GAME_PLATFORMS):
                    # Keep subscriptions that include a supported download.
                    subproducts.append(sub)
        return subproducts

    @staticmethod
    def _is_multigame_key(key: Key, product_category: str, blacklist: Iterable[str]) -> bool:
        if product_category != 'bundle':  # Only bundles are expected to expose multigame keys.
            return False
    ...
    def _split_multigame_key(key: Key) -> List[KeyGame]:
        ...
        for i, name in enumerate(names):
            # Multi-game packs place "and" before the last game name.
    ...
    def _get_key_infos(orders: list) -> List[KeyInfo]:
        ...
        for details in orders:
            for tpks in details['tpkd_dict']['all_tpks']:
                key = Key(tpks)
                try:
                    key.in_galaxy_format()  # Validate the Galaxy-specific format before using the subscription.
