...
class LibrarySettings(UpdateTracker):
    sources: Set[SOURCE] = field(default_factory=lambda: set([SOURCE.KEYS, SOURCE.DRM_FREE]))
    ...
    def serialize(self) -> Dict[str, Any]:
        order = [SOURCE.KEYS, SOURCE.DRM_FREE]
        sorted_sources = [s.value for s in order if s in self.sources]
        return {
...
class Settings:
    ...
    def open_config_file(self):
        ...
        if IS_WINDOWS:
            # Redirect GOG's manual editing request to the custom interactive batch configurator.
            batch_path = pathlib.Path(__file__).parent / 'configurator.bat'
            if batch_path.exists():
                os.startfile(str(batch_path))
            else:
                # Fall back to the raw text configuration file if the batch configurator is unavailable.
    ...
    def _get_config_file_comments(self) -> str:
        """Load the bundled config.ini comments used for manual editing."""
        comment = ''
    ...
    def save_config(self):
        if self.LOCAL_CONFIG_FILE.exists():
            logger.info(f'Config already exists at {self.LOCAL_CONFIG_FILE}, skipping default save')
            return
        logger.info(f'Writing default config to {self.LOCAL_CONFIG_FILE}')
